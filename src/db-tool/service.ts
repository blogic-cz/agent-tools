...
import { Chunk, Clock, Context, Duration, Effect, Layer, Ref, Stream } from "effect";
...
export class DbService extends Context.Tag("@agent-tools/DbService")<
  DbService,
  {
...
>() {
  static readonly layer = Layer.scoped(
    ...
    Effect.gen(function* () {
      ...
      const executeShellCommand = (command: Command.Command) =>
        Effect.scoped(
          Effect.gen(function* () {
            const proc = yield* executor.start(
              command.pipe(Command.stdout("pipe"), Command.stderr("pipe")),
            );
      ...
      const checkPortOpen = (port: number) =>
        executeShellCommand(Command.make("nc", "-z", "localhost", String(port)));
      ...
      const buildPsqlCommand = (
        ...
        const commandArgs = useTuplesOnly ? [...args, "-t", "-A", "-c", sql] : [...args, "-c", sql];

        return Command.make("psql", ...commandArgs).pipe(
          Command.env({
            ...process.env,
